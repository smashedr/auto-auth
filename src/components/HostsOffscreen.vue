...
<template>
  ...
  <div class="offcanvas offcanvas-start glass" tabindex="-1" :id="`${id}-offcanvas`" :aria-labelledby="`${id}-label`">
    <div class="offcanvas-header">
      <ThemeSwitch />
      <h5 class="offcanvas-title ms-2" :id="`${id}-label`">{{ i18n.t('options.savedCredentials') }}</h5>
      <button
        type="button"
        class="btn-close"
